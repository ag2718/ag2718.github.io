...
> Duc Hoang<b>\*</b>, Aarush Gupta<b>\*</b>, and Philip C. Harris. "KANELÉ: Kolmogorov–Arnold Networks for Efficient LUT-based Evaluation." *Proceedings of the 2026 ACM/SIGDA International Symposium on Field Programmable Gate Arrays*. ACM, 2026. [http://dx.doi.org/10.1145/3748173.3779202](http://dx.doi.org/10.1145/3748173.3779202)
>
> **[ICML 2026]**  
...
While training models in software and deploying them on FPGAs for ultrafast inference is extremely useful, we now go further and perform online learning on the FPGA. Although this has been underexplored and historically considered impractical, we demonstrate that the ideas of LUT-based KAN inference can be extended to enable *on-FPGA gradient updates*. This type of ultrafast learning is crucial for real-time control systems (e.g. quantum control and nuclear fusion) where adaptation needs to happen at *sub-microsecond timescales*.[^3]

> Unlike a CPU/GPU, which fetches weights from memory, calculates gradients, and writes them back, the FPGA implements the gradient update logic as a dedicated, parallel circuit that directly modifies the flip-flops (on-FPGA memory) storing the coefficients.
{: .prompt-info }
